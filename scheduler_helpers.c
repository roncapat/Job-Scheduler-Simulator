...
void load_job (FILE *logfile, FILE *dbgpipe, core *c, queue *ready){    NO_W(dbgpipe)
...
void unload_job (FILE *logfile, FILE *dbgpipe, core *c, queue *ready, queue * waiting, sched_policy sp, u_int *exited){    NO_W(dbgpipe)
    if (c->cur_proc != NULL){ //NULL only when core is started first time
        ...
        switch (c->cur_proc->status){
            case PROC_READY:
                switch (sp){
                    case RR_CRITERIA:
                        queue_insert_tail (ready, c->cur_proc); break;
                    case SJN_CRITERIA:
                        queue_insert_sjn (ready, c->cur_proc); break;
                }
                fprintf (logfile, "core%hu,%u,%hu,ready\n", c->id, c->clk, c->cur_proc->id);
                D( fprintf (dbgpipe,"UNLOAD:       Job #%hu re-enqueued\n", c->cur_proc->id) )
                break;
            case PROC_BLOCKED:
                queue_insert_latn (waiting, c->cur_proc);
...
void flush_waiting_queue (FILE *logfile, FILE *dbgpipe, core *c, queue *ready, queue *waiting, sched_policy sp){    NO_W(dbgpipe)
    int flushed = 0;
    process *p = NULL;
    while ((p = queue_remove_first_elegible(waiting, c)) != NULL){
        switch (sp){
            case RR_CRITERIA:
                queue_insert_tail (ready, p); break;
            case SJN_CRITERIA:
                queue_insert_sjn (ready, p); break;
        }
        p->status = PROC_READY;
...
void forward_clock (FILE *dbgpipe, core *c, queue *q){    NO_W(dbgpipe)
